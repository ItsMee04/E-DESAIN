<template>
    <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 space-y-4">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div>
                <h3 class="text-lg font-bold text-blue-950">Data Jenis Kelamin</h3>
                <p class="text-xs text-blue-950/60">Pengelolaan daftar jenis kelamin</p>
            </div>

            <div class="flex items-center gap-2.5">
                <!-- Search Box -->
                <div class="relative w-full sm:w-64">
                    <input v-model="searchQuery" type="text" placeholder="Cari jenis kelamin..."
                ...
                <button @click="$emit('open-add')"
                    ...
                    <Plus :size="16" />
                    <span>Tambah Jenis Kelamin</span>
                </button>
            </div>
        </div>
            ...
            <table class="w-full text-left text-sm text-blue-950">
                <thead class="bg-gray-50 text-xs font-semibold text-blue-950/60 uppercase tracking-wider">
                    <tr>
                        ...
                        <th class="px-4 py-3">Nama Jenis Kelamin</th>
                ...
                <tbody v-else-if="paginatedItems.length === 0" class="divide-y divide-gray-100">
                    <tr>
                        <td colspan="4" class="text-center py-6 text-xs text-blue-950/50">
                            Tidak ada data jenis kelamin yang ditemukan.
                        </td>
                    </tr>
                </tbody>
                ...
                <tbody v-else class="divide-y divide-gray-100">
                    <tr v-for="(item, index) in paginatedItems" :key="item.id" class="hover:bg-gray-50/50 transition">
                        ...
                        <td class="px-4 py-3.5 font-medium text-blue-950">{{ item.jeniskelamin }}</td>
