    ...
    <div v-if="isOpen" class="fixed inset-0 z-50 flex items-center justify-center bg-blue-950/40 backdrop-blur-xs p-4">
        <div class="bg-white rounded-2xl border border-gray-100 shadow-xl w-full max-w-md overflow-hidden">
            ...
            <form @submit.prevent="$emit('save')" class="p-6 space-y-4">
                <div>
                    <BaseInput v-model="form.pegawai" label="Nama Pengguna" type="text" placeholder="Contoh: John Doe"
                        :error="errors.pegawai" required @blur="$emit('validate-pengguna', 'pegawai')" />
                </div>

                <div>
                    <BaseInput v-model="form.email" label="Email" type="text" placeholder="Contoh: john.doe@example.com"
                        :error="errors.email" required @blur="$emit('validate-pengguna', 'email')" />
                </div>

                <div>
                    <BaseInput v-model="form.username" label="Username" type="text" placeholder="Contoh: JohnDoe"
                        :error="errors.username" required @blur="$emit('validate-pengguna', 'username')" />
                </div>

                <div>
                    <BaseInput v-model="form.password" label="Password" type="text" placeholder="Contoh: ********""
                        :error="errors.password" required @blur="$emit('validate-pengguna', 'password')" />
                </div>
