    ...
    <div v-if="isOpen" class="fixed inset-0 z-50 flex items-center justify-center bg-blue-950/40 backdrop-blur-xs p-4">
        <div class="bg-white rounded-2xl border border-gray-100 shadow-xl w-full max-w-md overflow-hidden">
            ...
            <form @submit.prevent="$emit('save')" class="p-6 space-y-4">
                <div>
                    <label class="block text-xs font-semibold text-blue-950/70 mb-1.5">Nama Pengguna *</label>
                    <input v-model="form.pegawai" type="text" readonly placeholder="Contoh: John Doe"
                        @blur="$emit('validate-pengguna')"
                    ...
                    <p v-if="errors.pegawai" class="mt-1.5 text-[11px] text-red-600">
                        {{ errors.pegawai }}
                    </p>
                </div>

                <div>
                    <label class="block text-xs font-semibold text-blue-950/70 mb-1.5">Email *</label>
                    <input v-model="form.email" type="email" placeholder="Contoh: john.doe@example.com"
                        @blur="$emit('validate-pengguna')"
                        class="w-full text-xs bg-gray-50 border border-gray-200 rounded-xl px-3.5 py-2.5 text-blue-950 focus:outline-none focus:border-[#B20600] focus:bg-white transition" />

                    <p v-if="errors.email" class="mt-1.5 text-[11px] text-red-600">
                        {{ errors.email }}
                    </p>
                </div>

                <div>
                    <label class="block text-xs font-semibold text-blue-950/70 mb-1.5">Username *</label>
                    <input v-model="form.username" type="text" placeholder="Contoh: JohnDoe"
                        @blur="$emit('validate-pengguna')"
                        class="w-full text-xs bg-gray-50 border border-gray-200 rounded-xl px-3.5 py-2.5 text-blue-950 focus:outline-none focus:border-[#B20600] focus:bg-white transition" />

                    <p v-if="errors.username" class="mt-1.5 text-[11px] text-red-600">
                        {{ errors.username }}
                    </p>
                </div>

                <div>
                    <label class="block text-xs font-semibold text-blue-950/70 mb-1.5">Password *</label>
                    <input v-model="form.password" type="password" placeholder="Contoh: ********"
                        @blur="$emit('validate-pengguna')"
                        class="w-full text-xs bg-gray-50 border border-gray-200 rounded-xl px-3.5 py-2.5 text-blue-950 focus:outline-none focus:border-[#B20600] focus:bg-white transition" />

                    <p v-if="errors.password" class="mt-1.5 text-[11px] text-red-600">
                        {{ errors.password }}
                    </p>
                </div>
